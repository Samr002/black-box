...
| `--connection-min-idle` | 4 (configurable, max 8) | Pre-warmed idle tunnels so new users connect instantly. **Keep low** — high values make the client open many idle sockets at once, which breaks the reverse-tunnel WS handshake (observed: 14 → 100% "connection closed", listener never registers; 1-8 healthy). Settable at install / Edit menu, capped at 8. See upstream #358 |
...
| `--connection-min-idle` | ۴ (قابل تنظیم، حداکثر ۸) | تونل‌های آمادهٔ از پیش‌گرم‌شده تا کاربر جدید فوری وصل شود. **پایین نگه دار** — مقدار بالا باعث می‌شود کلاینت چند سوکت idle همزمان باز کند که دست‌دادنِ WS تونل معکوس را می‌شکند (مشاهده شد: ۱۴ → ۱۰۰٪ «connection closed»، لیسنر ثبت نمی‌شود؛ ۱ تا ۸ سالم). هنگام نصب/منوی Edit، سقف ۸. طبق #358 |
